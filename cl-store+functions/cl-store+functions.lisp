...
(defbackend cl-store+functions
    ...
    :old-magic-numbers (1912923 1886611788 1347635532 1886611820 1414745155
                                1349740876 1884506444 1347643724 1349732684
                                1953713219 1416850499 1395477571)
    :extends (cl-store)
    :fields ((restorers :accessor restorers
                        :initform
                        (let ((cl-store-backend (find-backend 'cl-store))
...
(defstore-cl-store+functions (func function stream)
  (if-let ((obj (get-function-referrer func)))
    ;; A small hack. We don't want the backend-restore-object methods to
    ;; check circularities for obj since possible circularities for the
    ;; function "func" was already handled by the resolving-backend.
    ;; FIXME - This hack use internal information about cl-store
    ;; but no alternative was found - unless explictly call functions
    ;; store-type-object and restore-type-object.
    (cl-store::internal-store-object *current-backend* obj stream)
    ;; No information about the function was provided by the user
    ;; (e.g. macro st was not used) giving up to cl-store standard storable
    ;; methods (which stores the function name)
    (call-next-method)))
...
(defrestore-cl-store+functions (code-information stream)
  ;; Another little hack. During storage, the method for code information used
  ;; call-next-method for storing the class. This means that the instance for
  ;; 'code-information does not count as a new referrenciable value, since
  ;; it's the same object restored by this method.
  ;; FIXME - see FIXME above.
  (let ((obj (cl-store::internal-restore-object
              *current-backend* (get-next-reader *current-backend* stream)
              stream)))
...
(defmethod backend-store ((backend cl-store+functions) (stream stream) obj)
  ...
  (with-storable-functions-storage ()
    ;; Circular checking is REQUIRED - not only because the code of the
    ;; functions could contain gensyms. Closure internal representation
    ;; is not exactly circular, but it contains various references to the
    ;; same object, and these references are meant to be eq.
